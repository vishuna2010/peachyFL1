<template>
  <div class="bg-white p-5 rounded-lg shadow-md border border-neutral-200 flex items-start space-x-4 transition-all duration-300 ease-in-out hover:shadow-lg">
    ...
    <div class="flex-grow">
      ...
      <p class="text-2xl sm:text-3xl font-bold text-text-primary group-hover:text-brand-primary">{{ value }}</p>
      <div v-if="trend" class="text-xs mt-1 flex items-center" :class="trendDirection === 'up' ? 'text-green-600' : trendDirection === 'down' ? 'text-red-600' : 'text-text-secondary'">
