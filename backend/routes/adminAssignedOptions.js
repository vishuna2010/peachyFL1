    ...
    try {
      ...
      if (value_ids && value_ids.length > 0) {
        ...
        const insertPromises = value_ids.map(valueId => {
          return client.query(
            'INSERT INTO product_assigned_option_specific_values (product_assigned_option_id, product_option_value_id) VALUES ($1, $2) RETURNING *',
      ...
      await client.query('ROLLBACK');
      console.error(`[adminAssignedOptions PUT /:assignedOptionId/values] Error for assignedOptionId ${assignedOptionId}:`, error); // Keep this error log
