...
- [ ] **Error Handling & Validation:**
    ...
    - [~] Systematically review and expand input validation (`express-validator` or similar) for all API endpoints. (Example applied to admin product creation, admin suppliers, admin product stock update, admin stock levels, admin product inventory batches; further review pending for all other endpoints)
...
- [~] **Database Interactions & Services:**
    - [~] Enforce service layer pattern: Ensure all DB logic is within services, minimize direct `db.query` in route handlers. (Completed for Categories, Discounts, Suppliers modules; Product C/U/Stock/StockLevels/InvBatches refactored. `productService.getAllProducts` internally refactored for maintainability; pending for others)
